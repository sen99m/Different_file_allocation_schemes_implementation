...
unordered_map<int, pair<int,vector<Block*> > > file_descriptor; // key is file id, first value holds content size for that file, second value in pair is a pointer to the starting block of the file
...
int write_file(int file_id,bool update)
{
    string content;
    cin >> content;
    ...
    if(update) {//check whether last block is empty or not,if empty allocate some data in the last block
        int old_content_size = file_descriptor[file_id].first;
        i = old_content_size%block_size;
        if(i!=0) {//last block has some space
            Block *last_block = file_descriptor[file_id].second.back();
            while(i < block_size && index < content_size)
                last_block->data[i++] = content[index++];
        }
    }
    ...
    while (index < content_size)
    {
        ...
        file_descriptor[file_id].second.push_back(curr);
    }
...
int create(int file_id)
{
    ...
    return write_file(file_id,false);
}
...
int update(int file_id)
{
    ...
    return write_file(file_id,true);
}
...
int read_file(int file_id) {
    ...
    int num_blocks = file_descriptor[file_id].second.size();
    int i;
    int file_size = file_descriptor[file_id].first;
    int j = 0;
    for(i=0;i<num_blocks;i++) {
        Block *itr = file_descriptor[file_id].second[i];
...
int main()
{
    ...
    while (true)
    {
        ...
        cin >> option;
        int fid;
        //cin>>fid;//enter fid -1 when u want to quit
        switch (option)
        {
        case 0: // create file
            cin>>fid;
